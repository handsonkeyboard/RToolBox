...
# plot a line chart 
t_line <- t_data    %>% # set Year as the x-axis and Avg_Temp as the y-axis
...
ggsave("Outputs/Climate Visual/Temperature_Index.png", width = 8, height = 4)


# generate an animated line graph
t_line.animation <- t_data    %>% # set Year as the x-axis and Avg_Temp as the y-axis
                                  ggplot(aes(x = Year, y = Avg_Temp)) +
                                  # add a line graph to the plot 
                                  # set the color to a named category 1 for reference in the legend 
                                  # show.legend = FALSE -> not show this element's legend 
                                  ...
                                  theme(# remove the tick marks from both the x-axis and y-axis
                                        ...
                                        # eliminate any extra space around the legend
                                        legend.margin = margin(0, 0, 0, 0)) +
                                 # reveal each new time frame progressively
                                 transition_reveal(Year)
                                    

animated_line <- animate(t_line.animation, height = 400, width = 800, fps = 30, duration = 20, end_pause = 60, res = 100)

anim_save("Outputs/Climate Visual/Temperature_Index.gif", animated_line)
